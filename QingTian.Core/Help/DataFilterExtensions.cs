...
namespace QingTian.Core
{
    public static class DataFilterExtensions
    {
        ...
        public static ISugarQueryable<TEntity> ToDataFilter<TEntity>(this ISugarQueryable<TEntity> query, string parameter = "a", string field = "", FilterType type = FilterType.User)
        {
            ...
            if (expression != null && AppUser.UserId != 0)
            {
        ...
        public static async void CheckDataScope(this long field, FilterType type = FilterType.User)
        {
            ...
            if (!AppUser.IsSuperAdmin)
            {
                ...
                using (var scope = _scopeFactory.CreateScope())
                {
                    ...
                    var _sysUserService = App.GetService<Services.ISysUserService>(service);
                    var dataScopes = new List<long>();
                    switch (type)
                    {
                        case FilterType.User:
                            dataScopes = await _sysUserService.GetDataScopeIdUserList(AppUser.UserId);
                            break;
                        case FilterType.Org:
                            dataScopes = await _sysUserService.GetUserDataScopeIdList(AppUser.UserId);
                            break;
                    }
        ...
        public static async Task<List<long>> GetDataScopeIdList(FilterType type = FilterType.User)
        {
            ...
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                {
                    case FilterType.User:
                        return await _sysUserService.GetDataScopeIdUserList(AppUser.UserId);
                    case FilterType.Org:
                        return await _sysUserService.GetUserDataScopeIdList(AppUser.UserId);
                    default:
                        return await _sysUserService.GetUserDataScopeIdList(AppUser.UserId);
                }
            }
