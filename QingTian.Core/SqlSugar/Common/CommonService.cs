...
namespace QingTian.Core
{
    public class CommonService : ICommonService, IScoped
    {
        ...
        {
            List<EntityInfo> entityInfos = IsCache ? (await _cache.GetAsync<List<EntityInfo>>(ConstCache.CACHE_KEY_ENTITYINFO) ?? new List<EntityInfo>()) : new List<EntityInfo>();
            ...
            await _cache.SetAsync(ConstCache.CACHE_KEY_ENTITYINFO, entityInfos);
